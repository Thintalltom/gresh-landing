...
export function Header({ showWorks, setShowWorks }: { showWorks: boolean, setShowWorks: (value: boolean) => void }) {
  ...
  return (
    <header  className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-4 bg-[#000000]/90 backdrop-blur-sm border-b border-white/5 font-sans">
      ...
      <div className='flex gap-5'>
        ...
        <button onClick={scrollToWaitlist} className="px-6 py-2 text-sm font-bold text-[#0d2f28] bg-[#33ff66] rounded-full hover:opacity-90 transition-opacity">
          WAITLIST
        </button>
      </div>
    </header>);
